...
import WheatImage from "../Image/wheat.jpg";
import { ChevronLeftIcon, ChevronRightIcon } from '@heroicons/react/20/solid'

export default function WheatPost() {
  const [wheats, setWheat] = useState([]);
  const [active, setActive] = useState(1);
  const recordsPerPage = 10;
  const lastIndex = active * recordsPerPage;
  const firstIndex = lastIndex - recordsPerPage;
  const records = wheats.slice(firstIndex, lastIndex);
  const npage = Math.ceil(wheats.length / recordsPerPage)
  ...
  useEffect(() => {
    fetch("http://localhost:5000/api/wheat/").then((result) => {
    ...
    <>
      <div className="grid grid-rows-1 pt-20 md:grid-cols-2">
        <div className="xl:mt-16 mt-12 xl:ml-40 sm:ml-10">
          <h5 className="text-2xl font-bold">Wheat(Gehun)</h5>
          ...
          <div className="">
            ...
            <div>
              <span className="relative inline-flex items-center gap-3 px-2 rounded ">
                ...
                <span className="pl-2">Triticum</span>
            ...
            <div>
              <span className="relative inline-flex items-center gap-3 px-2 rounded ">
                ...
                <span className="italic">T. aestivum</span>
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {records.map((wheat, i) => (
                      <tr key={i}>
                        ...
                        <td className="py-4 pl-4 pr-3 border-collapse border border-slate-300 text-sm font-medium text-gray-900 whitespace-nowrap sm:pl-1">
                          {wheat.commonNameMetabolite}
                        </td>
                        <td className="py-4 pl-4 pr-3 border-collapse border border-slate-300 text-sm font-medium text-gray-900 sm:pl-1">
                          {wheat.IUPACNameMetabolite}
                        </td>
                        <td className="py-4 pl-4 pr-3 border-collapse border border-slate-300 text-sm font-medium text-gray-900 whitespace-nowrap sm:pl-1">
                          {wheat.molecularWeight}
                        </td>
                        <td className="py-4 pl-4 pr-3 border-collapse border border-slate-300 text-sm font-medium text-gray-900 whitespace-nowrap sm:pl-1">
                          {wheat.molecularFormula}
                        </td>
                        <td className="py-4 pl-4 pr-3 border-collapse border border-slate-300 text-sm font-medium text-gray-900 sm:pl-1">
                          {wheat.canonicalSmiles}
                        </td>
                        <td className="py-4 pl-4 pr-3 border-collapse border border-slate-300 text-sm font-medium text-gray-900 whitespace-nowrap sm:pl-1">
                          {wheat.references}
                        </td>
          ...
          <div className="hidden sm:flex sm:flex-1 sm:items-center sm:justify-between">
            <div>
              <p className="text-sm text-gray-700">
                ...
                <span className="font-medium">{wheats.length}</span> results
              </p>
            </div>
            <div>
